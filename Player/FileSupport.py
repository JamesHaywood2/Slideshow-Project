...
MissingImage = r"Slideshow-Project\Player\MissingImage.png"

ProgramIcon = r"Slideshow-Project\Player\Icon.ico"

ball = r"Slideshow-Project\Player\ball.jpg"
ball2 = r"Slideshow-Project\Player\ball2.png"
...
def getJPEG(folderPath: str, recursive: bool = False):
    ...
    if recursive:
        ...
        files = [os.path.join(dirPath, f) for dirPath, dirNames, filenames in os.walk(folderPath) for f in filenames if
                 os.path.splitext(f)[1] == '.jpg' or os.path.splitext(f)[1] == '.jpeg']
    else:
        # Same as above, but only for the folder
        files = [os.path.join(folderPath, f) for f in os.listdir(folderPath) if
                 os.path.splitext(f)[1] == '.jpg' or os.path.splitext(f)[1] == '.jpeg']
